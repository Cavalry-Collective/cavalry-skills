...
3. **Stories** — the cards. Each belongs to exactly **one activity + one phase**; order within a cell is its rank (top = first). Keep the text short and recognizable to a user (what they can do), not implementation detail.
...
2. Replace **only** two things: the `<title>` at the top (so the browser tab/gallery names it) and the JSON inside `<script id="data" type="application/json">…</script>`. Leave the CSS and engine untouched. Schema:
   ```json
   {
     ...
     "stories":    [{ "id": "s1", "activity": "a1", "phase": "ph1", "text": "…", "tags": ["AI"] }]
   }
   ```
...
4. Tell the user how to use it: **drag any card into another cell** to re-slice its phase/activity, and drop above/below other cards to rank it; **drag a phase rail or activity header** onto another to reorder rows/columns (moves animate; a ⠿ grip appears on hover); double-click text to edit; hover a card for ⌗ (tags) / ×; the dashed **＋ story / ＋ activity / ＋ phase** buttons in the grid grow it; **Copy to Clipboard / Export** save the new arrangement (they can paste it back so you regenerate the downstream plan/tasks), **Import** loads a saved one, **Reset** restores the original; the **EN | 中文** toggle at the top right switches the UI language.

## Notes

- **Self-contained** — no external fonts/scripts (Artifact CSP-safe). Drag-and-drop is native HTML5; edits persist per-map in the browser's `localStorage`, and **Reset** clears them back to what you published.
